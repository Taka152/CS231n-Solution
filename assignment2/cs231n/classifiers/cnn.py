...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W2'] = \
    np.random.randn(num_filters * (H / 2) * (W / 2), hidden_dim) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    output, cache1 = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
    output, cache2 = affine_relu_forward(output, W2, b2)
    output, cache3 = affine_forward(output, W3, b3)
    scores = output
